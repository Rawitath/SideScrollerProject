...
//    }
    ...
    public void setScreenAnchor(Vector2 anchor) {
        this.anchor = anchor;
    }
    ...
    public void draw(Graphics g, Vector2 posOffset, Vector2 scaleOffset, float zoom) {
        ...
        if(imageVisibled){
            ...
            Vector2 pos = getPosition().multiply(Vector2.negativeY()).add(anchor).add(getScreenAnchor())
                    .multiply(new Vector2((float)screen.width / (float)reference.getX(), (float)screen.height / (float)reference.getY())).add(posOffset);
